...
class CrawlAbuse(Task):
    def __init__(self):
        super().__init__('abuse 数据爬取')
    ...
    def save_info(self, ips, source, crawl_time):
        ...
        self.logger.info("abuse 共收集{0}条数据， 新数据{1}条".format(all_count, avail_count))
        self.logger.info("abuse 抓取时间{0}，数据遍历时间{1}".format(crawl_time, storage_time))
