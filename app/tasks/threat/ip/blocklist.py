...
class CrawlBlocklist(Task):
    ...
    def save_info(self, ips, source, crawl_time):
        ...
        self.logger.info("blocklist 共收集{0}条数据， 新数据{1}条".format(all_count, avail_count))
        self.logger.info("blocklist 抓取时间{0}，数据遍历时间{1}".format(crawl_time, storage_time))


if __name__ == "__main__":
    freebuf = CrawlBlocklist()
    freebuf.run_crawl()
