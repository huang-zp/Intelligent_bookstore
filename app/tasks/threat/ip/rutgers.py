...
class CrawlRutgers(Task):
    def __init__(self):
        super().__init__('rutgers 数据爬取')
        self.logger = ContextLogger('threat_ip')

    def run_crawl(self):
        start = time.time()
        ips = []
        url = 'https://report.cs.rutgers.edu/DROP/attackers'   #FQ
        source = 'report.cs.rutgers.edu'
    ...
    def save_info(self, ips, source, crawl_time):
        ...
        self.logger.info("rutgers 共收集{0}条数据， 新数据{1}条".format(all_count, avail_count))
        self.logger.info("rutgers  抓取时间{0}，数据遍历时间{1}".format(crawl_time, storage_time))


if __name__ == "__main__":
    freebuf = CrawlRutgers()
    freebuf.run_crawl()
